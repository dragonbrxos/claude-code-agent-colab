...
export class GrowthBook {
  ...
  async setEncryptedFeatures(encryptedString, encryptionKey, subtle) {
    subtle = subtle || globalThis.crypto && globalThis.crypto.subtle;
    if (!subtle) {
      throw new Error("No SubtleCrypto implementation found");
    }
    try {
      const key = await subtle.importKey("raw", base64ToBuf(encryptionKey), {
        name: "AES-CBC",
        length: 128
  ...
  getFeatureResult(key, value, source, ruleId) {
    let experiment = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : null;
    let result = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : null;
    const ret = {
      value,
      on: !!value,
      off: !value,
  ...
  evalFeature(id) {
    ...
    // Get the feature
    const feature = this.context.features[id];
